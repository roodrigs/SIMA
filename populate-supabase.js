...
async function populate() {
  console.log('--- Iniciando População Direta do Supabase (Formato Prisma 7) ---');
  
  const url = process.env.DIRECT_URL || process.env.DATABASE_URL;
  
  if (!url) {
    console.error('❌ Erro: DATABASE_URL ou DIRECT_URL não encontrada no .env');
    return;
  }

  // No Prisma 7, o formato de injeção manual é este (datasources no plural):
  ...
  try {
    ...
    console.log('2. Criando admin...');
    await prisma.user.create({ data: { username: 'admin', password: '123' } });

    console.log('3. Criando questões...');
